...
dt_fast_time = 1/fs;
t_fast = (0:dt_fast_time:(Tp + tau_max + padding)-(1/fs))';
t_chirp = (0:obj.dt_fast_time:obj.Tp - (1/obj.fs))';            
...
% define the transmitted LFM signal
rect_tx = abs(t_tx/Tp) <= 1/2;
tx_signal = rect_tx .* exp(1j * pi * (mu * t_chirp.^2));
...
% compensate for the match filter's group delay
range_array = range_array - (Tp * c / 4);
