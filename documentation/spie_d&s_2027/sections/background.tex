...
\noindent The relative position of a scattering point as a function of slow-time ($t_{m}$) can be defined as [\citenum{Xu2022}]:
...
\noindent where $\theta(t_{m})$ or $\theta_m$ is the angle of the target scatterer relative to the $y$-axis, and $[x_{k}, y_{k}]^{T}$ is the initial position of the $k$-th scatterer relative to the center of the rotation of the target. The range from the origin located at the radar to a scattering point is [\citenum{Xu2022}]:
\begin{equation}
    \begin{aligned}
...
This system is underdetermined and has infinitely many solutions, each existing in the ($N-M$)-dimensional null space of the sensing matrix. However, to treat this as a compressed sensing problem, if the latent image can be assumed $K$-sparse or containing $K$ non-zero values, the problem becomes $K$-dimensional where $M \geq K$. Additionally, if the sensing matrix is roughly orthogonal and incoherent, associated with the Restricted Isometric Property (RIP), the solution can be unique [\citenum{Majumdar_2018}]. The compressed sensing problem can then be formulated as:

\begin{equation}
    \begin{aligned}
    &\min \|x\|_{0} \quad \text{subject to} \quad \|y - Ax\|_{2}^{2} \leq \text{constant}\\
    \text{or} \quad &\min \|y - Ax\|^{2}_{2} \quad \text{subject to} \quad  \|x\|_{0} \leq \text{sparsity}
    \label{eq:l0}
    \end{aligned}
\end{equation}

There are many different compressed sensing techniques, and Greedy algorithms like Orthogonal Matching Pursuit (OMP) are effective for non-linear recovery [\citenum{Bostock_2017}]. For a Hermitian sensing matrix ($A$) where $A^{-1} = A^{H}$, OMP iterates through a defined sparsity ($K$) and finds the indices ($\Lambda$) that maximize the projection of the residual, initialized as the measurement ($s$), onto the basis vectors formed by $A$. The maximization of the residual selects the coefficients projected on the basis vectors formed by $A$, and the step $r \gets s - A_s\hat{\alpha}$ prevents the re-selection of the same indices. The coefficients $\hat{\alpha}$ are the least-squares approximation formed from the selected basis vectors $A_s$, and they populate the latent image on the support $\Lambda$. Algorithm \ref{algo:OMP} formally describes the OMP algorithm [\citenum{Majumdar_2018}]. It is written here in the notation of Section \ref{sec:methods} so that the two modifications AR-OMP makes, Algorithm \ref{algo:AROMP}, are visible as changes to single lines: the selected atom $a_\ell$ is replaced by the best of its ambiguous, off-grid candidates, and a cyclic refinement of every selected atom is inserted before the residual update.

\begin{algorithm}[H]
\caption{Orthogonal Matching Pursuit}\label{algo:OMP}
\begin{algorithmic}[1]
\Procedure{OMP}{$A, s, K, \tau$}
  \Comment{dictionary, measurement, sparsity, noise level}
\State $\Lambda \gets \emptyset$,\; $A_s \gets [\,]$,\; $r \gets s$
\For{$i = 1$ \textbf{to} $K$}
  ...
  \State $\ell \gets \arg\max_{j}|c_j|$,\quad $\Lambda \gets \Lambda \cup \{\ell\}$
    \Comment{detection}
  \State $A_s \gets [A_s,\, a_\ell]$
    \Comment{the selected atom, as it sits on the grid}
...
\EndFor
\State \textbf{return} $\hat{\alpha},\, \Lambda$
\EndProcedure
\end{algorithmic}
\end{algorithm}