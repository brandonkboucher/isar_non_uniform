classdef template_scenario < handle
    ...
    methods
        function obj = template_scenario(T)
            ...
            B   = 2*149.9 * const.MHz2Hz; % [Hz] bandwidth
            prf = 1 * const.kHz2Hz; % [Hz] pulse repetition frequency
            fs  = 2*300 * const.MHz2Hz; % [Hz] sampling frequency
            ...
            % intialize target
            obj.target = Target_Simple(...
                obj.signal.dt_slow, target_position, num_scatterers);
            
            % set straight line velocity towards the radar
            obj.target.velocity = [0, 0, 0];
            obj.target.body_axis_rotation_vel = [0,0,2*pi/T]; 
        end
