function [alpha_hat, p_hat, cand] = mod_omp_newton_2d(...
    s,...
    A,...
    sparsity,...
    grid,...
...
%   (Candidate positions are before cyclic refinement; p_hat is after.)
    ...
    % ambiguities
    num_offsets = ceil(grid.range_offset / (grid.range_pixel_res));

    y_offsets = ...
        -ceil(num_offsets)*grid.range_pixel_res ...
    ...
    for i = 1:sparsity
        ...
        seed = [x0 + AMB(:).'*grid.Wx + xOFF(:).'; ...
            y0 + yOFF(:).'];
        pend = zeros(2, num_of_candidates);
        corr = zeros(1, num_of_candidates);
        path = cell(1, num_of_candidates);
        best = 0; best_c = -inf;
        for ic = 1:num_of_candidates
            if record
