function plot_psf_comparison(...
    ...
    rx_signal_bp_norm_truth = ...
        truth_output.rx_signal_bp ...
        / max((truth_output.rx_signal_bp), [], "all");

    f = figure('Visible',visible);
    subplot(1,2,1)
    surf(x_array, y_array, abs(rx_signal_bp_norm) - abs(rx_signal_bp_norm_truth))
    ...
    title_str = create_title('PSF Change', omega, omega_dot);


    title(title_str, 'FontSize', 24, 'Interpreter','latex')

    subplot(1,2,2)
    imagesc(x_array, y_array, abs(rx_signal_bp_norm) - abs(rx_signal_bp_norm_truth))
    xlabel('x (cross-range)', 'FontSize', 16)
    ylabel('y (range)', 'FontSize', 16)
    axis square
    grid on
    ...
    if save_plots
        saveas(f, ['plots/psf_comp_', num2str(omega_dot), '.png'])
    end
